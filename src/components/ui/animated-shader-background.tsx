...
const AnimatedShaderBackground = () => {
  ...
  useEffect(() => {
    if (isMobile()) return; // skip WebGL on mobile — use CSS fallback
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          for(float i=0.0;i<20.0;i++) {
            ...
            o += col*exp(sin(i*i+iTime*0.8))/length(max(v,vec2(v.x*f*0.015,v.y*1.5)))*smoothstep(0.0,1.0,i/20.0)*0.6;
          }
    ...
    const animate = (ts: number) => {
      ...
      if (ts - last < 32) return; // cap at ~30fps
      last = ts;
  ...
  return (
    <div
      ...
      style={isMobile() ? {
        background: 'radial-gradient(ellipse at 60% 40%, #0a1628 0%, #000510 50%, #000000 100%)'
      } : undefined}
    />
  );
};
